...
class dbgame {
	...
	public function loadGamesForUpdate($competitionid, $processdate){
		...
		$query = $this->db->prepare("select game.gameid, game.team1score, game.team2score, game.result from game where game.competitionid = :competitionid and game.date <= :date");
		$query->execute(array("competitionid" => $competitionid, "date" => $processdate));
		$results = $query->fetchAll(PDO::FETCH_OBJ);
	...
	public function loadPredsByGame($gameid){
		...
		$query = $this->db->prepare("select prediction.predictionid,prediction.userid,prediction.team1score,prediction.team2score,prediction.result,prediction.multiplier,prediction.points,prediction.accuracy from prediction where prediction.gameid = :gameid");
	...
	public function createPredictions( $userid, $gameIdList, $team1scoreList, $team2scoreList, $starList, $heartList ) {
		...
		$query = $this->db->prepare("insert into prediction (`userid`,`gameid`,`team1score`,`team2score`,`result`,`multiplier`,`accuracy`) values (:userid,:gameid,:team1score,:team2score,:result,:multiplier,100) on duplicate key update team1score=:team1score,team2score=:team2score,result=:result,multiplier=:multiplier,accuracy=100");
		...
		for($i=0, $size=count($gameIdArray); $i<$size; $i++){
			...
			if(($team1ScoreArray[$i] != "") && ($team2ScoreArray[$i] != "")){	
				...
				if($team1ScoreArray[$i] > $team2ScoreArray[$i]){
					$result = 'hw';
				} else if($team1ScoreArray[$i] < $team2ScoreArray[$i]) {
					$result = 'aw';
				} else {
					$result = 'd';
				}	
	...
	public function savePredictions($predictions){
		$query = $this->db->prepare("update prediction set points=:points, accuracy=:accuracy where predictionid=:predictionid");
		
		foreach($predictions as $pred){
			$query->execute(array("points" => $pred->points, "accuracy" => $pred->accuracy, "predictionid" => $pred->predictionid));
		}
	}
	...
	public function enterResults($gameIdList, $team1ScoreList, $team2ScoreList){;
		...
		for($i=0, $size=count($gameIdArray); $i<$size; $i++){
			...
			if(($team1ScoreArray[$i] != "") && ($team2ScoreArray[$i] != "")){				
				...
				$query->execute(array("team1Score" => $team1ScoreArray[$i], "team2Score" => $team2ScoreArray[$i], "result" => $result, "gameid" => $gameIdArray[$i]));
			}
		}	
	}
